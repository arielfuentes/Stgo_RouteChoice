...
````{r, echo = F, fig.cap="Metodología General", fig.id = "Met", fig.cap.style = "Image Caption"}
fpar_5
````
...
````{r, echo = F, message = F, warnings = F, fig.cap='Paradas a Reposicionar', fig.id='Repos'}
tm_shape(zoi) + 
...
````{r, echo = F, message = F, warning = F, fig.cap='Ruta Más Corta', fig.id='Dijkstra'}
...
````{r, echo = F, fig.cap="Diagrama de Voronoi Ordinario", fig.id = "Vor", fig.cap.style = "Image Caption"}
fpar_iii
````
...
`````{r, echo = F, message = F, warning = F, fig.cap='Área de Estudio', fig.id='AEst'}
tm_shape(zones) +
...
`````{r, echo = F, message = F, warning = F, fig.cap='Generación de Demanda', fig.id='Gdem'}
DemZon <- st_join(zoi, 
...
Por los datos provistos en la base de datos de viajes a partir de las validaciones de la tarjeta inteligente (bip!), mediante el uso de consultas SQL, proceso de limpieza, imputación de datos, espacialización y geoprocesos se puede obtener la trayectoria de los viajes producidos al interior del área de estudio junto con los datos característicos de la movilidad.

Se obtienen un total de 10553 puntos (paradas y estaciones de metro) dentro del sistema de transporte público metropolitano, quedando 6703 dentro del área de estudio.

`````{r, echo = F, message = F, warning = F, fig.cap='Paradas', fig.id='Pts'}
tm_shape(nngeo::st_remove_holes(st_union(zoi))) +
    tm_grid(col = "white", lwd = 2, labels.size = .5, n.x = 4) +
...
````{r, echo = F, message = F, warning = F, fig.cap='Diagrama de Caja de Demanda', fig.id='DCajaDem'}
...
filter(var_pts, variables == "Demanda", valor < 250) %>%
    ...
    labs(title="Diagrama de Cajas de Demanda", x = "", y = "Validaciones") +
    theme(plot.title = element_text(hjust = 0.5), axis.text.x=element_blank(), axis.ticks.x=element_blank())
````
Fuente: Elaboración Propia

````{r, echo = F, message = F, warning = F, fig.cap='Diagrama de Caja de Densidad Poblacional', fig.id='DCajaDens'}
filter(var_pts, variables == "Densidad") %>%
    ggplot(aes(variables, valor, fill = "variables")) +
    geom_boxplot(width = .3, fill = "#FFC300") +
    labs(title="Diagrama de Cajas de Densidad Poblacional", x = "", y = "Densidad Poblacional") +
    theme(plot.title = element_text(hjust = 0.5), axis.text.x=element_blank(), axis.ticks.x=element_blank())
````
Fuente: Elaboración Propia

````{r, echo = F, message = F, warning = F, fig.cap='Diagrama de Caja de Tiempos', fig.id='DCajaT'}
...
````{r, echo = F, message = F, warning = F, fig.cap='Gráfico de Violín de Demanda', fig.id='ViolinDem'}
filter(var_pts, variables == "Demanda" & valor < 250) %>%
    ggplot(aes(variables, valor, fill = "variables")) +
    geom_violin(width = .3, fill = "lightblue") +
    labs(title="Gráfico de Violín de Demanda", y = "Validaciones") +
    theme(plot.title = element_text(hjust = 0.5), 
...
````{r, echo = F, message = F, warning = F, fig.cap='Gráfico de Violín de Densidad Poblacional', fig.id='ViolinDens'}
filter(var_pts, variables == "Densidad") %>%
    ggplot(aes(variables, valor, fill = "variables")) +
    geom_violin(width = .3, fill = "lightblue") +
    labs(title="Gráfico de Violín de Demanda", y = "Validaciones") +
...
````{r, echo = F, message = F, warning = F, fig.cap='Gráfico de Violín de Tiempos', fig.id='ViolinT'}
filter(var_pts, !variables %in% c("Demanda", "Densidad")) %>%
  ggplot(aes(variables, valor, fill = "variables")) +
  geom_violin(width = .3, fill = "lightblue") +
  xlab("") +
  ylab("Tiempo") +
  labs(title="Gráfico de Violín de Tiempos", y = "Tiempo") +
  theme(plot.title = element_text(hjust = 0.5))
````
Fuente: Elaboración Propia

````{r, , echo = F, message = F, warning = F, fig.cap='Distribución de Demanda', fig.id='DistrDem'}
filter(var_pts, variables == "Demanda") %>%
...
````{r, , echo = F, message = F, warning = F, fig.cap='Distribución de Densidad Poblacional', fig.id='DistrDens'}
filter(var_pts, variables == "Densidad") %>%
    ggplot(aes(valor, fill = variables, colour = variables)) +
    geom_density(alpha = .5, show.legend = F, color="black", fill="lightblue") +
...
````{r, , echo = F, message = F, warning = F, fig.cap='Distribución de Tiempos', fig.id='Distr'}
filter(var_pts, !variables %in% c("Demanda", "Densidad")) %>%
    ggplot(aes(valor, fill = variables, colour = variables)) +
...
Se aplica por tanto un modelo Random Forest con muestra de entrenamiento y muestra de validación, cuyos hiperparámetros son:
...
Por tanto, presenta un buen nivel de ajuste con una distribución normal del error.
...
Disponemos como datos para realizar la nueva zonificación las paradas 

## 4.3 Análisis de Sensibilidad

Capas de Análisis de Sensibilidad: Variables a incorporar, mcmc, método de clusterización y método de óptimo de número de clusters.
...
Considerar clusterización como 
\newpage
