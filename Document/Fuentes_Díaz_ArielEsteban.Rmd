...
````{r, echo = F, message = F, warning = F, fig.cap='Gráfico de Violín de Demanda', fig.id='ViolinDem', fig.topcaption=TRUE}
filter(var_pts, variables == "Demanda" & valor < 250) %>%
    ggplot(aes(variables, valor, fill = "variables")) +
...
````{r, echo = F, message = F, warning = F, fig.cap='Gráfico de Violín de Densidad Poblacional', fig.id='ViolinDens', fig.topcaption=TRUE}
filter(var_pts, variables == "Densidad") %>%
    ggplot(aes(variables, valor, fill = "variables")) +
    geom_violin(width = .3, fill = "lightblue") +
    labs(title="Gráfico de Violín de Demanda", y = "Validaciones") +
...
````{r, echo = F, message = F, warning = F, fig.cap='Gráfico de Violín de Tiempos', fig.id='ViolinT', fig.topcaption=TRUE}
filter(var_pts, !variables %in% c("Demanda", "Densidad")) %>%
  ggplot(aes(variables, valor, fill = "variables")) +
  geom_violin(width = .3, fill = "lightblue") +
  xlab("") +
...
## 4.3 Resultados y Análisis de Sensibilidad
...
Como resultado las 866 zonas se transforman en 4,107, es decir, las 483 zonas del área de estudio se transforman en 3,724.
...
a) Umbral de mínimo de paradas por zona.

Como primer análisis de sensibilidad es consideramos cuanto cambia la cantidad de zonas de la nueva zonificación si tomamos en cuenta solo aquellas que sobrepasen el primer quartil, es decir, se aplica el proceso para aquellas zonas que contengan mayor a 8 paradas.
